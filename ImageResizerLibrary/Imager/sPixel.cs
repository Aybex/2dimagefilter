...
public struct sPixel : ICloneable, ISerializable {
  ...
  public uint AbsDifference(sPixel pixel) {
    return _LUMINANCE_TRIGGER * (Luminance - pixel.Luminance).Abs()
           + _CHROMA_V_TRIGGER * (ChrominanceV - pixel.ChrominanceV).Abs()
           + _CHROMA_U_TRIGGER * (ChrominanceU - pixel.ChrominanceU).Abs();
  }
